...
$data_url = 'http://dixie.black-apple.biz/external_themes.php';
...
<div class="content-form">
  <form action="<?php printf(WWW.'admin/a?data=upload-theme'); ?>" method="post" class="form-content" enctype="multipart/form-data">
    <div class="input-parent">
      <div>Upload Theme <span style="font-size:13px;color:#379;" title="Requires ZIP file">(*.zip)</span></div>
    ...
    foreach($file['data'] as $id=>$data){
      ...
      if(isset($data['file_url'])){
        if(in_array($data['theme_name'],$themes)){
          echo '<div class="external-center"><div class="button">Installed</div></div>';
        }else{
          echo '<div class="external-center"><a href="'.WWW.'admin/a?data=add-external-theme&file_url='.$data['file_url'].'"><div class="button">Install</div></a><div class="external-free">Free</div></div>';
        }
      }elseif(isset($data['theme_code'],$data['kurs'],$data['price'])){
        if(in_array($data['theme_name'],$plugins)){
          echo '<div class="external-center"><div class="button">Purchased</div></div>';
        }else{
        ...
        echo '<div class="external-center"><div class="button">Private</div></div>';
      }
...
var themes = <?php print(json_encode($themes)); ?>;
  ...
  $('#external_themes').html('<div><img src="<?php print(WWW.PUBDIR); ?>admin/images/loader.gif" width="16px" height="11px" /> <span style="margin-left:5px;">Loading...</span></div>');
  $.get(external_url,function(result){
    if(result.status=='error'){
      ...
      for(var key in data){
        ...
        if(data[key].file_url!==undefined){
          if(themes[key_name]!==undefined){
            $('#'+id).append('<div class="external-center"><div class="button">Installed</div></div>');
          }else{
            $('#'+id).append('<div class="external-center"><a href="<?php print(WWW); ?>admin/a?data=add-external-theme&file_url='+data[key].file_url+'"><div class="button">Install</div></a><div class="external-free">Free</div></div>');
          }
        }else if(data[key].theme_code!==undefined&&data[key].price!==undefined&&data[key].kurs!==undefined){
          if(themes[key_name]!==undefined){
            $('#'+id).append('<div class="external-center"><div class="button">Purchased</div></div>');
          }else{
            $('#'+id).append('<div class="external-center"><a href="<?php print(WWW); ?>admin/a?data=purchase-external-theme&theme_code='+data[key].theme_code+'"><div class="button">Purchase</div></a><div class="external-price-theme">'+data[key].kurs+' '+number_format(data[key].price,0,',','.')+'</div></div>');
          }
        }else{
          $('#'+id).append('<div class="external-center"><div class="button">Private</div></div>');
        }
      ...
      $('#external_themes').html('unknown error');
    }
  });
</script>
<?php

...
<div class="content-form">
  <form action="<?php printf(WWW.'admin/a?data=new-menu'); ?>" method="post" class="form-content">
    <div class="config-body">
      ...
      <?php
        foreach($menu_types as $type){echo '<option value="'.$type.'">'.ucwords($type).'</option>';}
      ?>
      </select></div>
    </div>
    <div>Menu Name<input type="text" name="name" class="form-input" placeholder="Menu Name" list="list_title" /></div>
    <div>Menu Slug<input type="text" name="slug" class="form-input" placeholder="Menu Slug" list="list_slug" /></div>
    <div>Menu Order<input type="text" name="order" class="form-input" placeholder="Menu Order" /></div>
